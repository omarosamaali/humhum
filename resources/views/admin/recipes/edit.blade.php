...
<!-- ... (باقي الملف كما هو) -->
                ...
                <div class="col-md-12 mb-3">
                    ...
                    <select class="form-control select2" name="sub_categories[]" id="sub_categories"
                        multiple="multiple">
                    ...
                    if (stepText) {
                        ...
                        $this.find('.media-item').each(function() {
                            ...
                            if (imgSrc && imgSrc.includes('storage/')) {
                                ...
                                existingMedia.push({ url: urlPath, type: 'image', original_name: 'existing_image.jpg' });
                            } else if (videoSrc && videoSrc.includes('storage/')) {
                                const urlPath = videoSrc.split('storage/')[1];
                                existingMedia.push({ url: urlPath, type: 'video', original_name: 'existing_video.mp4' });
                            }
                        });
                        steps.push({ description: stepText, media: existingMedia });
                    }
                });
                if (steps.length === 0) {
                    Swal.fire({ icon: 'error', title: 'خطأ', text: 'يجب إضافة خطوة واحدة على الأقل!' });
                    return;
                }
                ...
                $.ajax({
                    ...
                    error: function(xhr) {
                        ...
                        if (xhr.status === 422) {
                            ...
                            errors = response.errors ? Object.values(response.errors).flat() : [response.message || 'خطأ في التحقق من البيانات'];
                            errorMessage = errors.join('<br>');
                        } else if (xhr.status === 500) {
                            errorMessage = 'خطأ في الخادم: ' + (xhr.responseJSON?.message || 'يرجى التحقق من السجلات');
                        } else {
                            errorMessage = xhr.responseJSON?.message || 'خطأ غير معروف';
                        }
