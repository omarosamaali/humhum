...
@section('content')
    <div class="add-section">
        <h5 class="mb-4">
            ...
            تعديل صفحة معلومات عنا: {{ $types->title_ar }}
        </h5>
        ...
        @endif
        {{-- 
        <form action="{{ route('admin.about-us.update', $types) }}" method="POST"
            enctype="multipart/form-data"> --}}
        <form action="{{ route('admin.types.update', $types->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <input type="text" class="form-control" id="title_ar" name="title_ar"
                            value="{{ old('title_ar', $types->title_ar) }}" required>
                        @error('title_ar')
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        <textarea class="form-control" id="description_ar" name="description_ar" rows="4" required>{{ old('description_ar', $types->description_ar) }}</textarea>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        @enderror
                        @if ($types->main_image)
                            <img id="current_main_image_preview" src="{{ $types->main_image_url }}" alt="الصورة الحالية"
                        ...
                        @endif
                        <img id="main_image_preview" src="#" alt="معاينة الصورة الجديدة" class="about-preview mt-2"
                            style="display: none;">
                    </div>
                </div>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        ...
                        @enderror
                        @if ($types->sub_image)
                            <img id="current_sub_image_preview" src="{{ $types->sub_image_url }}" alt="الصورة الحالية"
                                class="about-preview mt-2 p-2 current-image-preview">
                            <div class="form-check mt-2">
                        ...
                        @endif
                        <img id="sub_image_preview" src="#" alt="معاينة الصورة الجديدة" class="about-preview mt-2"
                            style="display: none;">
                    </div>
                </div>
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="status" id="status" required>
                            <option value="1" {{ old('status', $types->status) == '1' ? 'selected' : '' }}>فعال
                            </option>
                            <option value="0" {{ old('status', $types->status) == '0' ? 'selected' : '' }}>غير
                                فعال</option>
                        </select>
                        @error('status')
                            <div class="text-black">{{ $message }}</div>
                ...
                @foreach ($targetLanguages as $code => $name)
                    <div class="col-md-12 mb-3 border rounded-lg p-2">
                        ...
                        <span class="text-right">
                            {{ old('title_' . $code, $types->{'title_' . $code}) }}
                        </span>
                        <br>
                        <label for="description_{{ $code }}" class="form-label font-bold">{{ $name }}
                            (الوصف):</label>
                        <span class="text-right">
                            {{ old('description_' . $code, $types->{'description_' . $code}) }}
                        </span>
                    </div>
                @endforeach
            </div>
