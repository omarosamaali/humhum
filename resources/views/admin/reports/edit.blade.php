    ...
    <div class="add-section">
        ...
        <form action="{{ route('admin.packages.update', $package->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="mb-3">
                        <label for="name_ar" class="form-label font-bold">اسم الباقة (بالعربية)</label>
                ...
                <div class="col-md-6">
                    <div class="mb-3">
                        <label for="image_input" class="form-label font-bold">صورة الباقة</label>
                        ...
                        @if ($package->image)
                            <img id="current_image_preview" src="{{ $package->image_url }}" alt="الصورة الحالية"
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="status" id="status" required>
                            <option value="1" {{ old('status', $package->status) == '1' ? 'selected' : '' }}>فعال
                            </option>
                            <option value="0" {{ old('status', $package->status) == '0' ? 'selected' : '' }}>غير فعال
                            </option>
                        </select>
                        @error('status')
                            <div class="text-black">{{ $message }}</div>
            ...
            <div class="row">
                @foreach ($targetLanguages as $code => $name)
                    <div class="col-md-12 mb-3 border rounded-lg p-2">
                        ...
                        {{-- الوصول للترجمة مباشرة من عمود الـ Model --}}
                        {{-- <input type="text" class="form-control translated-name-field" id="name_{{ $code }}"
                            value="" readonly> --}}
                            <span class="text-right">
                                {{ old('name_' . $code, $package->{'name_' . $code}) }}
                            </span>
                    </div>
                @endforeach
            </div>
