...
@section('page-title', 'تعديل الإرشاد')
    ...
    <div class="add-section">
        ...
        <form action="{{ route('admin.tips.update', $tip->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-md-6">
                    <div class="mb-3">
                        <label for="name_ar" class="form-label font-bold">عنوان الإرشاد (بالعربية)</label>
                        <input type="text" class="form-control" id="name_ar" name="name_ar"
                            value="{{ old('name_ar', $tip->name_ar) }}" required>
                    </div>
                </div>

                {{-- <div class="col-md-6">
                    <div class="mb-3">
                        <label for="image_input" class="form-label font-bold">صورة الإرشاد</label>
                        ...
                        @if ($tip->image)
                            <img id="current_image_preview" src="{{ $tip->image_url }}" alt="الصورة الحالية"
                                class="tip-preview mt-2 p-2 current-image-preview">
                        ...
                        @endif
                        <img id="image_preview" src="#" alt="معاينة الصورة الجديدة" class="tip-preview mt-2"
                            style="display: none;">
                    </div>
                </div> --}}
                ...
                <div class="col-md-4">
                    <div class="mb-3">
                        ...
                        <select class="form-select" name="status" id="status" required>
                            <option value="1" {{ old('status', $tip->status) == '1' ? 'selected' : '' }}>فعال
                            </option>
                            <option value="0" {{ old('status', $tip->status) == '0' ? 'selected' : '' }}>غير فعال
                            </option>
                        </select>
                        @error('status')
                            <div class="text-black">{{ $message }}</div>
            ...
            <h6 class="mt-4 mb-3 text-black">العناوين المترجمة (للقراءة فقط - تتحدث تلقائياً):</h6>
            <div class="row">
