...
<body class="bg-light">
    <div class="page-wrapper">
        ...
        <main class="page-content">
            ...
            <form id="ingredientsForm" method="POST"
                action="{{ route('c1he3f.recpies.updateIngredients', $recipe->id) }}">
                @csrf
                @method('PUT')
    ...
    <script>
        ...
        `;
            ...
            @if (isset($ingredientsData) && !empty($ingredientsData))
                @foreach ($ingredientsData as $ing)
                    createIngredientItem("{{ addslashes($ing['description']) }}",
                        {{ $ing['is_heading'] ? 'true' : 'false' }});
                @endforeach
            @else
                if ($('#ingredients-list .ingredient-item').length === 0) {
                    createIngredientItem();
