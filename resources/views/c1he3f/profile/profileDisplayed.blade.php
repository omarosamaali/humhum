...
<body>
    <div class="page-wrapper">
        ...
        <main class="page-content space-top p-b40">
            <div class="container pt-0">
                <div class="profile-area">
                    <div class="author-bx">
                        <div class="dz-media" style="position: relative; height: 246px; width: 282px;">
                            <img style="border-radius: 15px;" src="{{ asset('storage/' . Auth::user()->chefProfile->official_image ) }}" alt>
                            @php
                            function getCountryFlag($country) {
                            $flags = [
                            'مصر' => ['code' => 'eg', 'emoji' => '🇪🇬'],
                            'السعودية' => ['code' => 'sa', 'emoji' => '🇸🇦'],
                            'الإمارات' => ['code' => 'ae', 'emoji' => '🇦🇪'],
                            'الأردن' => ['code' => 'jo', 'emoji' => '🇯🇴'],
                            'المغرب' => ['code' => 'ma', 'emoji' => '🇲🇦'],
                            'الجزائر' => ['code' => 'dz', 'emoji' => '🇩🇿'],
                            'السودان' => ['code' => 'sd', 'emoji' => '🇸🇩'],
                            'تونس' => ['code' => 'tn', 'emoji' => '🇹🇳'],
                            'لبنان' => ['code' => 'lb', 'emoji' => '🇱🇧'],
                            'قطر' => ['code' => 'qa', 'emoji' => '🇶🇦']
                            ];

                            return $flags[$country] ?? ['code' => 'un', 'emoji' => '🏳️'];
                            }

                            $userCountry = Auth::user()->chefProfile->country;
                            $flag = getCountryFlag($userCountry);
                            @endphp

                            <!-- استخدام الصورة -->
                            <img src="https://flagcdn.com/64x48/{{ $flag['code'] }}.png" alt="علم {{ $userCountry }}" class="flag--img inline-block mr-2">

                            <!-- أو استخدام الإيموجي -->
                            <span class="text-xl">{{ $flag['emoji'] }}</span> </div>

                        <div class="dz-content">
                            <h2 class="name">الطاهي / {{ Auth::user()->name }}</h2>
                            <p class="text-primary">
                                {{ Auth::user()->chefProfile->bio }}
                            </p>
                        </div>
                    </div>
        ...
        <!-- Menubar -->
        <div class="menubar-area footer-fixed" style="direction: rtl;">
