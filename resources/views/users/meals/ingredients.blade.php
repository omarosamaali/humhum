...
<body class="bg-light">
            ...
            @if (count($ingredients) > 0)
                @foreach ($ingredients as $ingredient)
                    ...
                    @if (Str::startsWith($ingredient, '##'))
                        <div
                            style="display: flex; gap: 10px; align-items: center; justify-content: space-between; font-weight: bold; border-bottom: 1px solid var(--primary); padding-bottom: 5px;">
                            {{ $textToSpeak }} 🍴
                            <div style="display: flex; gap: 5px; align-items: center;">
                                <div onclick="sendUnavailableNotification('{{ addslashes($textToSpeak) }}', event)"
                                    ...
                                    title="إرسال إشعار المكون غير متوفر">
    ...
    <script>
        function sendUnavailableNotification(componentName, event) {
            const buttonElement = event.currentTarget;
            buttonElement.style.pointerEvents = 'none';
            buttonElement.style.opacity = '0.6';
            buttonElement.style.cursor = 'not-allowed';

            fetch('/send-unavailable-notification', {
                    method: 'POST',
                    headers: {
                ...
                .then(data => {
                    if (data.success) {
                        buttonElement.innerHTML =
                            '<i style="font-size: 20px; color: #ffffff;" class="fa-solid fa-check"></i>';
                        buttonElement.style.backgroundColor = '#28a745';
                        buttonElement.title = "تم الإرسال بنجاح";
                        Swal.fire({
                            title: "تم الإرسال",
                            text: "تم إرسال المكون الناقص بنجاح",
                            icon: "success"
                        });
                    } else {
                        ...
                        Swal.fire({
                            title: "خطأ",
                            text: data.message || 'حدث خطأ في إرسال الإشعار',
                            icon: "error"
                        });
                    }
                })
                .catch(error => {
                    ...
                    Swal.fire({
                        ...
                        text: "حدث خطأ في الإتصال بالخادم",
                        icon: "error"
                    });
                });
        }
