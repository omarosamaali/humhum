...
<body class="bg-light">
    <div class="page-wrapper">
        ...
        @endif
{{-- إصلاح الـ View --}}
<form action="{{ route('chef_lens.submit-response', $challenge->id) }}" method="POST" enctype="multipart/form-data">
    @csrf
    ...
    <main class="page-content space-top p-b100" style="direction: rtl;">
        <div style="border: 1px solid var(--primary); margin-top: 16px; width: 87%; border-radius: 16px; margin-bottom: 10px;" class="container">
            <div style="display: flex; justify-content: center;">
                <div class="dz-item-rating" style="border-radius: 50px; background-color: #e00000; font-size: 17px; overflow: hidden; line-height: unset; border: 2px solid #e00000;">
                    {{-- ✅ إصلاح عرض صورة الشيف --}}
                    @if($challenge->chefProfile && $challenge->chefProfile->official_image)
                    <img src="{{ asset('storage/' . $challenge->chefProfile->official_image) }}" style="width: 60px; height: 60px;" alt="صورة الشيف">
                    @else
                    <div style="width: 60px; height: 60px; background: #ccc; display: flex; align-items: center; justify-content: center;">
            ...
            <span style="align-items: center; justify-content: center; display: flex;">
                {{ $challenge->message ?? 'عنوان التحدي' }}
            </span>
            <p style="text-align: center; font-size: 12px; margin-bottom: 0px;">
                تاريخ البدء: {{ \Carbon\Carbon::parse($challenge->start_date)->format('Y-m-d') }}
                <br>
        ...
        <h6 style="text-align: center;">
            اسم الوصفة المختارة: {{ $challenge->recipe?->title ?? 'لا توجد وصفة مربوطة' }}
        </h6>
        ...
        <div class="container">
            ...
            <div class="bg-cookpad-gray-9gi p-6h1" style="height: 40vh; width: 80%; margin: auto; border-radius: 15px;">
                <div style="top: 20%;" class="image-zyn text-wbi fle-kj4 item-sji justify-byc">
                    ...
                    <input type="file" name="recipe_image" id="recipe-image-input" accept="image/*" class="form-control">
                </div>
                @error('recipe_image')
            ...
            <div class="bg-cookpad-gray-9gi p-6h1" style="margin-top: 20px !important; height: 40vh; width: 80%; margin: auto; border-radius: 15px;">
                <div style="top: 20%;" class="image-zyn text-wbi fle-kj4 item-sji justify-byc">
                    ...
                    <input type="file" name="challenge_video" id="challenge-video-input" accept="video/*" required class="form-control">
                </div>
            ...
            {{-- التنبيهات --}}
            <span style="display: flex; justify-content: center; font-size: 12px; color: var(--primary); font-weight: bold; text-align: center;">
                يجب التركيز علي تصوير طريقة إعداد الطبخة وعدم إبراز ملامح الوجه والمكان
            </span>
            <span style="display: flex; justify-content: center; font-size: 12px; color: var(--primary); font-weight: bold; text-align: center;">
                جميع الصور والفيديوهات والمحتوي المرفوع من مسؤلية المستخدم
            </span>

            {{-- رسالة للطاهي --}}
            <div class="my-3">
                <textarea name="message_to_chef" id="message-to-chef" style="height: 98px; text-align: center; color: #000000;" placeholder="رسالة إلي الطاهي" class="form-control">{{ old('message_to_chef') }}</textarea>
...
{{-- إضافة JavaScript للتحقق من الملفات --}}
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        form.addEventListener('submit', function(e) {
            if (!videoInput.files.length) {
                e.preventDefault();
                alert('يرجى تحديد فيديو التحدي');
                return false;
            }

            // التحقق من حجم الفيديو (300MB = 300 * 1024 * 1024)
            const videoFile = videoInput.files[0];
            if (videoFile.size > 300 * 1024 * 1024) {
                e.preventDefault();
                alert('حجم الفيديو كبير جداً. يجب أن لا يزيد عن 300 ميجابايت');
                return false;
            }
        });
    });
