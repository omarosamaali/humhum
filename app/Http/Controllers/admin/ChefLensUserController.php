...
class ChefLensUserController extends Controller
{
    ...
    {
        $query = User::where('system', 'مستخدم')->latest();
        $query->with(['chefProfile' => function ($q) {}]);
        if ($request->has('role') && $request->role != '') {
            $query->where('role', $request->role);
        }
        $users = $query->paginate(10);
        return view('admin.chefLensUsers.index', compact('users'));
    }
    ...
    {
        return view('admin.chefLensUsers.create');
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->role === 'طاه') {
            ...
            Auth::login($user);
            return redirect()->route('admin.chefLensUsers.index')
    ...
    public function show(User $user)
    {
        ...
        return view('admin.chefLensUsers.show', compact('user'));
    }

    public function edit(User $user)
    {
        return view('admin.chefLensUsers.edit', compact('user'));
    }
